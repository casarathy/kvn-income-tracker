...
st.markdown("""
    <style>
        html, body, [class*="css"], .stMarkdown, p, h1, h2, h3, h4, h5, h6, label, table, div, [data-testid="stDataFrame"] * {
        ...
        if uploaded_csv is not None and st.button("Process CSV File", type="primary"):
            try:
                ...
                if not missing_cols:
                    ...
                    st.error(f"Schema layout mismatch. Missing columns: {', '.join(missing_cols)}")
            except Exception as e: 
